...
console.log("Display the size: " + size(SSL) + "\n");

//  isEmpty: finds if the list is empty or not (without using the size() function)
const isEmpty = (linkList) => {
  return !linkList.head;
};
console.log("Is empty?: " + isEmpty(SSL) + "\n");

//  findPrevious: finds the node before the item you are looking for
const findPrevious = (linkList, target) => {
...
console.log("Find Previous with 'Kat' input:\n  " + JSON.stringify(findPrevious(SSL, "Kat")) + "\n");

//  findLast: returns the last node in the linked list
const findLast = (linkList) => {
  // If our head is null, the linked list is empty
  if ( !linkList.head ) {
    return null;
  }
